...
PUMAClass = namedtuple('PUMAClass', ['name', 'id', 'has_instances', 'color'])
classes = [
    PUMAClass('Background',           0, True,    (0,         0,      0)), # black
    PUMAClass('Lymphocyte',               1, True,    (255,       0,      0)), # red
    PUMAClass('Tumor',           2, True,    (255,       255,    0)), # green
    PUMAClass('Others',           3, True,    (0,         255,    0)), # orange
]
num_classes = 4
mapping_id = torch.tensor([x.id for x in classes])
colors = torch.tensor([cls.color for cls in classes])

GLYSAC_CLASSES = ['', 'lymphocyte', 'tumor', 'others']
...
class PumaDataset(Dataset):
    def __init__(
        ...
        elif 'puma' in augmentation_type:
            self.augmentation = Compose([
                RandomHorizontalFlip(p=0.5),

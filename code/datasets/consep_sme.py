...
CoNSePClass = namedtuple('CoNSePClass', ['name', 'id', 'has_instances', 'color'])
classes = [
    CoNSePClass('Background',           0, True,  (  0,   0,   0)), # black
    CoNSePClass('Others',               1, True,  (255, 255,   0)), # red
    CoNSePClass('Inflammatory',         2, True,  (255,   0, 255)), # green
    CoNSePClass('Epithelial',           3, True,  (255,   0,   0)), # orange
    CoNSePClass('Spindle',              4, True,  (0,     0, 255)), # purple
]
num_classes = 5
mapping_id = torch.tensor([x.id for x in classes])
colors = torch.tensor([cls.color for cls in classes])

CONSEP_CLASSES = ['', 'others', 'inflammatory', 'epithelial', 'spindle']
...
class ConsepDataset(Dataset):
    def __init__(
        ...
        elif 'consep' in augmentation_type:
            self.augmentation = Compose([
                RandomHorizontalFlip(p=0.5),
    ...
    def set_prompt(self, cls):
        ...
        if len(cls_now)==1:
            ...
            included = []
            for i in cls_now[1:]:
                included.append(CONSEP_CLASSES[int(i)])
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            target_np = np.array(original_pil_target)
            class_image = np.zeros((image_np.shape[0], image_np.shape[1], 5, image_np.shape[2]), dtype=np.uint8)

            for region_id in range(5):
                mask = (target_np == region_id)
                mask = np.expand_dims(mask, axis=-1)
                class_image[:, :, region_id, :] = np.where(mask, image_np, 0)

            class_image = class_image.reshape(class_image.shape[0], class_image.shape[1], -1).astype(np.uint8)

        except:
            print(f"An exception occurred trying to load file {self.images[idx]}.")
        # Transforms
        image = ToTensor()(original_pil_image)
        class_image = ToTensor()(class_image)
        ...
        img_dist_lbl_pnt = self.augmentation(torch.cat([image, dist, label, point, class_image]))

        aug_img_dist = img_dist_lbl_pnt[:5]
        aug_lbl = img_dist_lbl_pnt[5].unsqueeze(0)
        aug_pnt = img_dist_lbl_pnt[6].unsqueeze(0)
        class_image = img_dist_lbl_pnt[7:]

        caption = self.set_prompt(label)

        return aug_img_dist, aug_lbl, caption, aug_pnt, class_image

...
GLySACClass = namedtuple('GLySACClass', ['name', 'id', 'has_instances', 'color'])
classes = [
    GLySACClass('Background',           0, True,    (0,         0,      0)), # black
    GLySACClass('Others',               1, True,    (255,       0,      0)), # red
    GLySACClass('Lymphocyte',           2, True,    (255,       255,    0)), # green
    GLySACClass('Epithelial',           3, True,    (0,         255,    0)), # orange
]
num_classes = 4
mapping_id = torch.tensor([x.id for x in classes])
colors = torch.tensor([cls.color for cls in classes])

GLYSAC_CLASSES = ['', 'others', 'lymphocyte', 'epithelial']
...
class GlysacDataset(Dataset):
    def __init__(
        ...
        elif 'glysac' in augmentation_type:
            self.augmentation = Compose([
                RandomHorizontalFlip(p=0.5),
    ...
    def set_prompt(self, cls):
        ...
        if len(cls_now)==1:
            ...
            included = []
            for i in cls_now[1:]:
                included.append(GLYSAC_CLASSES[int(i)])
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            class_image = np.zeros((image_np.shape[0], image_np.shape[1], 4, image_np.shape[2]), dtype=np.uint8)

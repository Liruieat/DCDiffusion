...
class JointImagenTrainer(nn.Module):
    ...
    def sample(self, *args, **kwargs):
        ...
        _, _, caption, point, class_image = next(self.valid_dl_iter)
        caption = caption[:4]
        point = point[:4]
        class_image = class_image[:4]

        with context():
            output = self.imagen.sample(point=point, texts=caption, class_image=class_image, *args, device=self.device, **kwargs)

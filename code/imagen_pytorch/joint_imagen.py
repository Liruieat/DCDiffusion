...
class ImageEmbedding(nn.Module):
    ...
    def forward(self, x):
        assert x.dim() == 4, f'x.shape should be (B, 1, H, W) but {x.shape}'
        assert x.size(1) == 3, f'x.shape should be (B, 1, H, W) but {x.shape}'
        x = self.conv(x)
        x = self.emb_layer(x.long().squeeze(1))
        x = x.permute(0, 3, 1, 2)
        return x
class LinearCrossAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, reduction_factor=4, cosine_sim_attn=True):
        """
        Args:
            embed_dim (Tensor): Parameter tensor.
            num_heads (Tensor): Parameter tensor.
            reduction_factor (Tensor): Parameter tensor.
            cosine_sim_attn (Tensor): Parameter tensor.
        """
    ...
    def forward(self, x, y, mask=None, attn_bias=None):
        """
        Args:
            x (Tensor): Parameter tensor.
            y (Tensor): Parameter tensor.
            mask (Tensor): Parameter tensor.
            attn_bias (Tensor): Parameter tensor.
        Returns:
            output (Tensor): Parameter tensor.
        """
        batch_size, channels_x, height_x, width_x = x.size()
        _, channels_y, height_y, width_y = y.size()
        x = F.avg_pool2d(x, kernel_size=4, stride=4)  # (batch_size, channels_x, height_out, width_out)
        y = F.avg_pool2d(y, kernel_size=4, stride=4)  # (batch_size, channels_y, height_out, width_out)
        batch_size_out, _, height_out, width_out = x.size()
        x_seq = x.view(batch_size_out, channels_x, -1).transpose(1, 2)  # (batch_size, sequence_length, embed_dim)
        y_seq = y.view(batch_size_out, channels_y, -1).transpose(1, 2)  # (batch_size, sequence_length, embed_dim)
        Q_prime = self.q_proj(x_seq)  # (batch_size, sequence_length, reduced_dim)
        K_prime = self.k_proj(y_seq)  # (batch_size, sequence_length, reduced_dim)
        V = self.v_proj(y_seq)        # (batch_size, sequence_length, embed_dim)
        scale = self.reduction_factor ** -0.5 if not self.cosine_sim_attn else 1.
        attn_scores = torch.matmul(Q_prime, K_prime.transpose(-2, -1)) * scale  # (batch_size, sequence_length, sequence_length)
        attn_weights = F.softmax(attn_scores, dim=-1)  # (batch_size, sequence_length, sequence_length)
        attn_output = torch.matmul(attn_weights, V)  # (batch_size, sequence_length, embed_dim)
        attn_output = self.out_proj(attn_output)  # (batch_size, sequence_length, embed_dim)
        output = attn_output.transpose(1, 2).contiguous()  # (batch_size, sequence_length, embed_dim)
        output = output.view(batch_size_out, channels_x, height_out, width_out)  # (batch_size, embed_dim, height_out, width_out)
        output_upsampled = F.interpolate(
            output,
            size=(height_x, width_x),
            mode='bilinear',
            align_corners=False
        )
...
class JointUnet(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.init_emb_cls = ImageEmbedding(self.num_classes, channels_lbl)
        self.init_emb_seg_lowres = LabelEmbedding(self.num_classes, channels_lbl) if lowres_cond else None
        self.init_emb_cls_ca = LinearCrossAttention(3, 1)
        ...
        self.final_conv_2 = nn.Conv2d(final_conv_dim_in, 2,
                                    final_conv_kernel_size, padding=final_conv_kernel_size // 2)
        ...
        self.dist_conv = nn.Sequential(nn.Conv2d(2,16,1), nn.Conv2d(16,64,1))
        ...
        self.cls_conv = nn.Sequential(nn.Conv2d(3,16,1), nn.Conv2d(16,64,1))
    ...
    def forward_with_cond_scale(
        ...
        **kwargs
        ...
        if cond_scale == 1:
            logits, logits2, logits_seg = self.forward(*args, point_map=point, class_image=class_image, **kwargs)
            # logits, logits2, logits_seg = self.forward(*args, point_map=point, **kwargs)
            return logits, logits2, logits_seg
        elif cond_scale == 0:
            null_logits, null_logits2, null_logits_seg = self.forward(*args, point_map=point, class_image=class_image, cond_drop_prob=1., **kwargs)\
    ...
    def forward(
        ...
        # joint imagen
        lbl_c = lbl.long().clone()
        lbl = self.init_emb_seg(lbl.long())
        # x = torch.cat((x, dist, lbl), dim=1) # 3+1+3=7
        x = self.img_conv(x)
        dist = self.dist_conv(dist)
        lbl = self.lbl_conv(lbl)
        # cls_cuts = torch.split(class_image, 3, dim=1) 
        # cls_embs = [self.cls_conv(cut) for cut in cls_cuts]
        # sum_emb = sum(cls_embs)
        # x = x + dist + lbl + sum_emb        
        x = x + dist + lbl 
        # condition on self
        # if self.self_cond:
        ...
        # if exists(lbl_c) and exists(class_image):    
        #     lbl_embs = []
        #     # for cut in torch.unique(lbl_c):
        #     #     mask = (lbl_c == cut).float()
        #     #     lbl_embs.append(self.init_emb_pnt(mask))
        #     lbl_embs.append(self.init_emb_pnt(lbl_c))
        #     emb_lbl = self.init_emb_pnt(lbl_c)
        #     lbl_embs_feature = self.rrdb(emb_lbl)
        #     cls_cuts_ = torch.split(class_image, 3, dim=1) 
        #     # cls_embs_ = [self.init_emb_cls(cut) for cut in cls_cuts_]
        #     cls_embs_ = []
        #     cls_embs_.append(self.init_emb_cls(torch.stack(cls_cuts_, dim=0).sum(dim=0)))
        #     ca_embs = []
        #     for lbl_emb, cls_emb in zip(lbl_embs, cls_embs_):
        #         ca_embs.append(self.init_emb_cls_ca(lbl_emb,cls_emb))
        #     rrdb_embs = [self.rrdb(cut) for cut in ca_embs]
        #     cls_map_feature = sum(rrdb_embs)
        #     x = x + cls_map_feature + lbl_embs_feature + point_map_feature
        #     cls_map_residual = x.clone()
        if exists(lbl_c) and exists(class_image):    
            lbl_embs = []
            for cut in torch.unique(lbl_c):
                mask = (lbl_c == cut).float()
            ...
            for cut in torch.unique(lbl_c):
                mask = (lbl_c == cut).float()
                lbl_embs.append(self.rrdb(self.init_emb_pnt(mask)))
            emb_lbl = self.init_emb_pnt(lbl_c)
            lbl_embs_feature = self.rrdb(emb_lbl)
            # ## dropout
            # B,C,H,W = point_map_feature.shape
            ...
            # point_map_feature = dropout(point_map_feature)
            # print(emb_point.shape, point_map_feature.shape, x.shape)
            x = x + lbl_embs_feature
            lbl_embs_residual = x.clone()
        elif exists(class_image):
            cls_cuts_ = torch.split(class_image, 3, dim=1) 
            # cls_embs_ = [self.init_emb_cls(cut) for cut in cls_cuts_]
            # cls_sum_emb = sum(cls_embs_)
            # cls_map_feature = self.rrdb(cls_sum_emb)
            cls_embs_ = [self.rrdb(self.init_emb_cls(cut)) for cut in cls_cuts_]
            cls_map_feature = sum(cls_embs_)
        # init conv residual
        ...
        # #     for lbl_emb, cls_emb in zip(lbl_embs, cls_embs_):
        # #         ca_embs.append(self.init_emb_cls_ca(lbl_emb,cls_emb))
        # #     cls_map_feature = sum(ca_embs)
        # #     x = x + cls_map_feature + lbl_embs_feature
        # #     cls_map_residual = x.clone()
        # if exists(lbl_c) and exists(class_image):    
        #     lbl_embs = []
        #     for cut in torch.unique(lbl_c):
        ...
        #     rrdb_embs = [self.rrdb(cut) for cut in ca_embs]
        #     cls_map_feature = sum(rrdb_embs)
        #     x = x + cls_map_feature + lbl_embs_feature + point_map_residual
        #     cls_map_residual = x.clone()
        # elif exists(lbl_c):
        #     lbl_embs = []
        #     for cut in torch.unique(lbl_c):
        ...
        # if exists(point_map) and exists(class_image):
        #     point_map_residual = point_map_residual + cls_map_residual
        #     x = torch.cat((x, point_map_residual), dim=1)
        # elif exists(point_map):
        #     x = torch.cat((x, point_map_residual), dim=1)
        # elif exists(class_image):
        #     x = torch.cat((x, cls_map_residual), dim=1)
        if exists(class_image):
            x = torch.cat((x, cls_map_residual), dim=1)
        # 함
    ...
    def forward_with_cond_scale_prototype(
        self,
        *args,
        cond_scale=1.,
        point=None,
        ...
        **kwargs
            ...
            rrdb_embs = self.forward_prototype(*args, point_map=point, class_image=class_image, **kwargs)
            # logits, logits2, logits_seg = self.forward(*args, point_map=point, **kwargs)
            return rrdb_embs
    def forward_with_cond_scale_prototype_sample(
        self,
        *args,
        cond_scale=1.,
        point=None,
        ...
        **kwargs
        ...
        if cond_scale == 1:
            logits, logits2, logits_seg = self.forward_prototype_sample(*args, point_map=point, class_image=class_image, prototype=prototype,**kwargs)
            # logits, logits2, logits_seg = self.forward(*args, point_map=point, **kwargs)
            return logits, logits2, logits_seg
        elif cond_scale == 0:
            null_logits, null_logits2, null_logits_seg = self.forward_prototype_sample(*args, point_map=point, class_image=class_image, prototype=prototype, cond_drop_prob=1., **kwargs)\
            # null_logits, null_logits2, null_logits_seg = self.forward(*args, point_map=point, cond_drop_prob=1., **kwargs)
            return null_logits, null_logits2, null_logits_seg
        else:
            logits, logits2, logits_seg = self.forward_prototype_sample(*args, point_map=point, class_image=class_image, prototype=prototype, **kwargs)
    ...
    def forward_prototype(
        ...
        if exists(lbl_c) and exists(class_image):    
            ...
            # for lbl_emb, cls_emb in zip(lbl_embs, cls_embs_):
            #     # ca_embs.append(self.init_emb_cls_ca(lbl_emb,cls_emb))
            #     ca_embs.append(cls_emb)
            # rrdb_embs = [self.rrdb(cut) for cut in ca_embs]
        return cls_embs_
    def forward_prototype_sample(
        self,
        x,
        dist,
        lbl,
        ...
        if exists(lbl_c) and exists(class_image):    
            ...
            cls_embs_ = [self.init_emb_cls(cut) for cut in cls_cuts_]
            for cut in torch.unique(lbl_c):
                cosine_sorce = F.cosine_similarity(cls_embs_[cut].reshape(1, -1), prototype[cut].reshape(1, -1), dim=1)
                cls_embs_[cut] = cosine_sorce*cls_embs_[cut] + (1-cosine_sorce)*prototype[cut]
                # 
            ca_embs = []
            for lbl_emb, cls_emb in zip(lbl_embs, cls_embs_):
                ca_embs.append(self.init_emb_cls_ca(lbl_emb,cls_emb))
...
class JointImagen(nn.Module):
    ...
    def p_mean_variance(
        ...
        *,
        ...
        pred, pred_dist, pred_lbl = default(model_output, lambda: unet.forward_with_cond_scale(
            ...
            text_embeds=text_embeds, text_mask=text_mask, point=point,class_image=class_image,
        ...
        if pred_objective == 'noise':
            ...
            # lbl_start = noise_scheduler_lbl.predict_start_from_noise(log_lbl, t=t, noise=pred_lbl)
    ...
    def p_sample(
        ...
        *,
        ...
        (model_mean, _, model_log_variance), pred_lbl, x_start, lbl_start, (model_mean_2, _, model_log_variance_2), dist_start = self.p_mean_variance(
            unet, x=x, dist=dist, log_lbl=log_lbl, t=t, t_next=t_next, point=point,class_image=class_image,
    ...
    def p_sample_loop(
        ...
        *,
        ...
        dist = torch.randn((batch,2,256,256), device=device)
        ...
        in tqdm(zip(range(1, num_unets + 1), self.unets, self.sample_channels, self.image_sizes,
            ...
            with context:
                ...
                if exists(unet_init_images) and exists(unet_init_labels):
                    ...
                    unet_init_labels = self.resize_to(unet_init_labels, image_size)
                shape = (batch_size, self.channels, *frame_dims, *image_size)
                rrdb_embs = self.p_sample_loop_prototype(
                    unet,
                    shape,
                    point=point,
                    class_image=class_image,
                ...
                # outputs.append((rrdb_embs.cpu()))
                outputs.append((rrdb_embs))
            if exists(stop_at_unet_number) and stop_at_unet_number == unet_number:
                break
        # # either return last unet output or all unet outputs
        # output_index = -1 if not return_all_unet_outputs else slice(None)
        ...
        in tqdm(zip(range(1, num_unets + 1), self.unets, self.sample_channels, self.image_sizes,
            ...
            with context:
                ...
                if exists(unet_init_images) and exists(unet_init_labels):
                    ...
                    unet_init_labels = self.resize_to(unet_init_labels, image_size)
                shape = (batch_size, self.channels, *frame_dims, *image_size)
                img, lbl = self.p_sample_loop_prototype_sample(
                    unet,
                    shape,
                    point=point,
                    class_image=class_image,
        ...
        *,
        ...
        for times, times_next in tqdm(timesteps, desc='sampling loop time step', total=len(timesteps), disable=not use_tqdm):
            ...
            for r in reversed(range(resample_times)):
                ...
                self_cond = x_start if unet.self_cond else None
                self_cond_lbl = lbl_start if unet.self_cond else None
                rrdb_embs = self.p_sample_prototype(
                    unet=unet,
                    x=img,
                    dist=dist,
                    log_lbl=log_lbl,
        ...
        return rrdb_embs
    @torch.no_grad()
    def p_sample_loop_prototype_sample(
        self,
        unet,
        shape,
        *,
        ...
        for times, times_next in tqdm(timesteps, desc='sampling loop time step', total=len(timesteps), disable=not use_tqdm):
            ...
            for r in reversed(range(resample_times)):
                ...
                self_cond = x_start if unet.self_cond else None
                self_cond_lbl = lbl_start if unet.self_cond else None
                img, dist, log_lbl, x_start, dist_start, lbl_start = self.p_sample_prototype_sample(
                    unet=unet,
                    x=img,
                    dist=dist,
                    log_lbl=log_lbl,
    ...
    def p_sample_prototype(
        ...
        *,
        ...
        b, *_, device = *x.shape, x.device
        rrdb_embs = self.p_mean_variance_prototype(
            unet, x=x, dist=dist, log_lbl=log_lbl, t=t, t_next=t_next, point=point,class_image=class_image,
            noise_scheduler=noise_scheduler, noise_scheduler_lbl=noise_scheduler_lbl, noise_scheduler_2 = noise_scheduler_2,
            text_embeds=text_embeds, text_mask=text_mask,
            cond_images=cond_images, cond_scale=cond_scale,
    ...
    def p_sample_prototype_sample(
        ...
        *,
        ...
        b, *_, device = *x.shape, x.device
        (model_mean, _, model_log_variance), pred_lbl, x_start, lbl_start, (model_mean_2, _, model_log_variance_2), dist_start = self.p_mean_variance_prototype_sample(
            unet, x=x, dist=dist, log_lbl=log_lbl, t=t, t_next=t_next, point=point,class_image=class_image, prototype=prototype,
            noise_scheduler=noise_scheduler, noise_scheduler_lbl=noise_scheduler_lbl, noise_scheduler_2 = noise_scheduler_2,
            text_embeds=text_embeds, text_mask=text_mask,
            cond_images=cond_images, cond_scale=cond_scale,
            lowres_cond_img=lowres_cond_img, lowres_cond_lbl=lowres_cond_lbl,
    ...
    def p_mean_variance_prototype(
        ...
        *,
        ...
        assert not (cond_scale != 1. and not self.can_classifier_guidance), 'imagen was not trained with conditional dropout, and thus one cannot use classifier free guidance (cond_scale anything other than 1)'
        lbl = log_onehot_to_index(log_lbl)
        rrdb_embs = default(model_output, lambda: unet.forward_with_cond_scale_prototype(
            x, dist, lbl, noise_scheduler.get_condition(t),
            text_embeds=text_embeds, text_mask=text_mask, point=point,class_image=class_image,
            cond_images=cond_images, cond_scale=cond_scale,
            lowres_cond_img=lowres_cond_img, lowres_cond_lbl=lowres_cond_lbl,
            ...
            lowres_noise_times=self.lowres_noise_schedule.get_condition(lowres_noise_times)))
        return rrdb_embs
    def p_mean_variance_prototype_sample(
        self,
        unet: JointUnet,
        x,
        dist,
        ...
        *,
        ...
        assert not (cond_scale != 1. and not self.can_classifier_guidance), 'imagen was not trained with conditional dropout, and thus one cannot use classifier free guidance (cond_scale anything other than 1)'
        lbl = log_onehot_to_index(log_lbl)
        pred, pred_dist, pred_lbl = default(model_output, lambda: unet.forward_with_cond_scale_prototype_sample(
            x, dist, lbl, noise_scheduler.get_condition(t),
            text_embeds=text_embeds, text_mask=text_mask, point=point,class_image=class_image,prototype=prototype,
            cond_images=cond_images, cond_scale=cond_scale,
            lowres_cond_img=lowres_cond_img, lowres_cond_lbl=lowres_cond_lbl,
            self_cond=self_cond, self_cond_lbl=self_cond_lbl,
            lowres_noise_times=self.lowres_noise_schedule.get_condition(lowres_noise_times)))
    ...
    def p_losses(
        ...
        *,
        ...
        # noise_lbl = default(noise_lbl, lambda: torch.randn_like(x_start))
        # normalize to [-1, 1]
        ...
        if self_cond and random() < 0.5:
            with torch.no_grad():
                ...
                #     lbl_noisy, t=times, noise=pred_lbl) if pred_objective == 'noise' else pred_lbl
                lbl_start = None
    ...
    def forward(
        ...
        dists = img_dist[:,3:,:]
        cond_images = maybe(cast_uint8_images_to_float)(cond_images)
        class_imgs = cast_uint8_images_to_float(class_image)
        ...
        class_imgs = self.resize_to(class_imgs, target_image_size)
        return self.p_losses(unet, images, dists, labels, times, text_embeds=text_embeds, text_mask=text_masks, cond_images=cond_images, noise_scheduler=noise_scheduler, noise_scheduler_2=noise_scheduler_2, noise_scheduler_lbl=noise_scheduler_lbl, lowres_cond_img=lowres_cond_img, lowres_cond_lbl=lowres_cond_lbl, lowres_aug_times=lowres_aug_times, pred_objective=pred_objective, p2_loss_weight_gamma=p2_loss_weight_gamma, random_crop_size=random_crop_size,
                             point=points, class_image=class_imgs)

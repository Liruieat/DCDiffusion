...
class JointImagenTrainer_Test(nn.Module):
    locked = False
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        assert not JointImagenTrainer_Test.locked, 'JointImagenTrainer can only be initialized once per process - for the sake of distributed training, you will now have to create a separate script to train each unet (or a script that accepts unet number as an argument)'
        assert exists(imagen) ^ exists(
            imagen_checkpoint_path), 'either imagen instance is passed into the trainer, or a checkpoint path that contains the imagen config'
        ...
        JointImagenTrainer_Test.locked = self.is_distributed
    ...
    def add_train_dataset(self, ds=None, *, batch_size, **dl_kwargs):
        ...
        if self.split_valid_from_train:
            ...
            ds, valid_ds = valid_ds, ds
            # ds, valid_ds = random_split(ds, [train_size, valid_size],
            #                             generator=torch.Generator().manual_seed(self.split_random_seed))
            self.print(
                f'training with dataset of 0 samples and validating with randomly splitted {len(valid_ds)} samples')
    ...
    @cast_torch_tensor
    @imagen_sample_in_chunks
    def sample(self, point, *args, **kwargs):
        context = nullcontext if kwargs.pop('use_non_ema', False) else self.use_ema_unets
        ...
        # _, _, _, point = next(self.valid_dl_iter)
        # point = point[:4]
        # point = args.points

        with context():
            output = self.imagen.sample(point=point, *args, device=self.device, **kwargs)
    ...
    @cast_torch_tensor
    @imagen_sample_in_chunks
    def sample_prototype(self, point, *args, **kwargs):
        context = nullcontext if kwargs.pop('use_non_ema', False) else self.use_ema_unets
        ...
        with context():
            output = self.imagen.sample_prototype(point=point, *args, device=self.device, **kwargs)
    ...
    @cast_torch_tensor
    @imagen_sample_in_chunks
    def sample_prototype_sample(self, point, *args, **kwargs):
        context = nullcontext if kwargs.pop('use_non_ema', False) else self.use_ema_unets
        ...
        with context():
            output = self.imagen.sample_prototype_sample(point=point, *args, device=self.device, **kwargs)
